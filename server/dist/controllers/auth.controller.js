...
exports.getSessions = exports.logoutAll = exports.logout = exports.refreshToken = exports.verifyOtp = exports.sendOtp = exports.getMe = exports.login = exports.register = void 0;
const prisma_1 = __importDefault(require("../utils/prisma"));
const zod_1 = require("zod");
const crypto_1 = __importDefault(require("crypto"));
const auth_service_1 = require("../services/auth.service");
const axios_1 = __importDefault(require("axios"));
const phid_seeder_1 = require("../utils/phid.seeder");
const registerSchema = zod_1.z.object({
    email: zod_1.z.string().email(),
    password: zod_1.z.string().min(6),
    role: zod_1.z.enum(['PATIENT', 'PROVIDER', 'ADMIN']).default('PATIENT'),
...
const register = async (req, res) => {
    try {
        ...
        const passwordHash = await auth_service_1.AuthService.hashPassword(validatedData.password);
        ...
        const newUser = await prisma_1.default.$transaction(async (tx) => {
            ...
            if (validatedData.role === 'PATIENT') {
                const uniqueIdSegment = crypto_1.default.randomBytes(4).toString('hex').toUpperCase();
        ...
        if (validatedData.role === 'PATIENT') {
            const profile = await prisma_1.default.patientProfile.findUnique({ where: { userId: newUser.id } });
            if (profile && profile.healthId) {
                // Goal 3: Dynamically pre-populate the whole database ecosystem for this PHID
                await (0, phid_seeder_1.generateMockEcosystemForPHID)(profile.id, profile.healthId);
                // Attach the profile back to the user object for the response
                registeredUser.patientProfile = profile;
            }
        }
        // Generate access and refresh tokens
        const tokenPair = auth_service_1.AuthService.generateTokenPair({
            id: registeredUser.id,
            email: registeredUser.email,
            role: registeredUser.role,
        });
        // Create session
        await auth_service_1.AuthService.createSession(registeredUser.id, registeredUser.email, registeredUser.role, tokenPair.accessToken, tokenPair.refreshToken, {
            ipAddress: req.ip,
    ...
    try {
        ...
        if (email.endsWith('@mediscore.local')) {
            ...
            if (!user) {
                ...
                user = await prisma_1.default.user.findUnique({ where: { email }, include: { patientProfile: true } });
            }
        }
        else {
            user = await prisma_1.default.user.findUnique({ where: { email }, include: { patientProfile: true } });
        ...
        const isMatch = await auth_service_1.AuthService.verifyPassword(password, user.passwordHash);
        if (!isMatch) {
        ...
        // Create session
        await auth_service_1.AuthService.createSession(user.id, user.email, user.role, tokenPair.accessToken, tokenPair.refreshToken, {
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'],
            deviceType: 'web',
        });
...
const sendOtp = async (req, res) => {
    try {
        ...
        const existingOtp = await prisma_1.default.otpVerification.findFirst({
            where: { identifier }
        });
        if (existingOtp) {
...
const refreshToken = async (req, res) => {
    try {
        ...
        const newTokenPair = await auth_service_1.AuthService.refreshAccessToken(refreshToken, '');
        if (!newTokenPair) {
            res.status(401).json({
        ...
        await auth_service_1.AuthService.refreshAccessToken(refreshToken, newTokenPair.accessToken);
        res.json({
...
const logout = async (req, res) => {
    try {
        ...
        if (token) {
            await auth_service_1.AuthService.invalidateSession(token);
        }
...
const logoutAll = async (req, res) => {
    try {
        ...
        await auth_service_1.AuthService.invalidateAllUserSessions(req.user.id);
