...
exports.getEmiSchedules = exports.applyForCredit = exports.payBill = exports.getBillingRecords = void 0;
...
const getBillingRecords = async (req, res) => {
    try {
        ...
        const creditImpact = pendingBills.reduce((sum, b) => sum + (b.creditImpact || 0), 0);
        const totalExpenses = bills.reduce((sum, b) => sum + b.billAmount, 0);
        ...
        const creditScoreContribution = patientProfile.timelineEvents.reduce((sum, e) => sum + (e.impact > 0 ? e.impact : 0), 0);
        // Format for frontend
...
const payBill = async (req, res) => {
    try {
        ...
        const profile = await prisma_1.default.patientProfile.findUnique({ where: { userId } });
        if (!profile)
            return;
        const bill = await prisma_1.default.billingRecord.findUnique({
            where: { id: billId }
        });
...
const applyForCredit = async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        if (!profile)
            return;
        const bill = await prisma_1.default.billingRecord.findUnique({ where: { id: billId } });
        ...
        const newApp = await prisma_1.default.$transaction(async (tx) => {
...
const getEmiSchedules = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!profile)
            return;
        const apps = await prisma_1.default.creditApplication.findMany({
            where: { patientId: profile.id },
            include: {
                billingRecord: true,
                emiPlan: {
...
exports.getEmiSchedules = getEmiSchedules;
