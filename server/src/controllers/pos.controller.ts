...
export const verifyPOS = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { phid } = req.body;
...
export const checkoutPOS = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const offer = await prisma.offerApplication.create({
            data: {
                patientId: profile.id,
