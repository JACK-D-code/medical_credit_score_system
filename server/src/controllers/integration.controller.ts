...
export const syncProvider = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const hasSynced = patient.timelineEvents.some((e: any) => e.type === 'metadata' && e.title === `Synced ${provider}`);
        if (hasSynced) {
