...
import { io } from '../index'; // Import the global socket io instance

export const calculateScoreLogic = async (patientId: string) => {
    const patient = await prisma.patientProfile.findUnique({
        where: { userId: patientId },
        include: { financialProfile: true }
    });

    const patAny = patient as any;
    ...
    let riskLevel: 'EXCELLENT' | 'GOOD' | 'FAIR' | 'HIGH_RISK' = 'HIGH_RISK';
    if (finalScore >= 800) riskLevel = 'EXCELLENT';
    else if (finalScore >= 650) riskLevel = 'GOOD';
    else if (finalScore >= 500) riskLevel = 'FAIR';

    // Store the historical calculation in the database
    const newScoreRecord = await prisma.medicalCreditScore.create({
        data: {
            patientId: patient.id,
            scoreValue: finalScore,
            paymentHistoryScore: healthScore,      
            incomeStabilityScore: financialScore, 
            medicalDebtScore: Math.min(100, (patient as any).financialProfile?.existingMedicalDebt === 0 ? 100 : 50),
    ...
    if (io) {
        io.to(`user_room_${patientId}`).emit('score_updated', {
            newScore: finalScore,
            category: riskLevel,
...
export const calculateScore = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const result = await calculateScoreLogic(patientId);
        res.json(result);
    } catch (error: any) {
...
export const getScoreHistory = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const history = await prisma.medicalCreditScore.findMany({
            where: { patientId: patient.id },
...
export const getScoreDetails = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
