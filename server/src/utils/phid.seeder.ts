...
export const generateMockEcosystemForPHID = async (patientId: string, healthId: string) => {
    try {
        ...
        await prisma.dynamicCreditScore.create({
            data: {
                patientId,
        ...
        for (let i = 0; i < numClaims; i++) {
            ...
            await prisma.creditClaimRequest.create({
                data: {
                    patientId,
        ...
        for (let i = 0; i < numOffers; i++) {
            ...
            await prisma.offerApplication.create({
                data: {
                    patientId,
                    offerType,
