...
filepath = 'C:/Users/acer/Desktop/Project/server/src/controllers/provider.controller.ts'

with open(filepath, 'r', encoding='utf-8') as f:
    code = f.read()

# The exact function to inject
new_function = """export const issueBill = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
...
pattern = r"export const issueBill \= async \(req\: AuthRequest\, res\: Response\)\: Promise\<void\> \=\> \{[\s\S]*?(?=\nexport const)"

new_code = re.sub(pattern, new_function + "\n", code, count=1)
